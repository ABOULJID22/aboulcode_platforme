...
<head>
  ...
  <title>{{ $post->title }} · ABOULCODE</title>
 ...
 @include('layouts.navbar')
  ...
  <main class="min-h-screen py-10 sm:py-14">
    <div class="max-w-5xl mx-auto px-6 lg:px-8">
      ...
      <!-- Contenu -->
      @php
        $renderedContent = \Filament\Forms\Components\RichEditor\RichContentRenderer::make($post->content ?? null)
            ->toHtml();
      @endphp
          ...
          <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-5">
            ...
            <div class="flex flex-wrap gap-2 sm:gap-3">
              ...
              <a href="mailto:?subject={{ rawurlencode($post->title) }}&body={{ rawurlencode($url) }}"
                 class="share-button bg-primary-500/10 text-primary-700 dark:text-primary-200 dark:hover:text-white w-full sm:w-auto">
                {{ __('site.blog.share.email') }}
              </a>
              <button type="button"
                      x-data
                      x-on:click="navigator.clipboard.writeText('{{ $url }}')"
                      class="share-button bg-gray-100 text-gray-800 hover:bg-primary-500 hover:text-white dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-primary-500 w-full sm:w-auto">
